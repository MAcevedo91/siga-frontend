...
import { getEstudianteById, descargarPDF } from '@/services/estudiantesService'
...
export default function EstudiantePerfilPage() {
  ...
  const canExportPDF = ['Administrador', 'Equipo de Formación'].includes(user?.rol)

  const handleDescargarPDF = async () => {
    try {
      setLoadingPDF(true)
      const fecha = new Date().toISOString().split('T')[0]
      const rut = estudiante.rut.replace(/[^0-9kK]/g, '')
      await descargarPDF(id, `historial_${rut}_${fecha}.pdf`)
    } catch (err) {
              ...
              <div className="flex items-start justify-between">
                ...
                {canExportPDF && (
                  <button
                    ...
                    disabled={loadingPDF || !estudiante.incidentes?.length}
                    title={!estudiante.incidentes?.length ? 'Sin incidentes para exportar' : 'Descargar historial PDF'}
                    className="flex items-center gap-2 rounded-lg bg-white/10 px-4 py-2 text-sm font-medium text-white hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-50 transition-all"
                  >
                    {loadingPDF ? (
                      <>
